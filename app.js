...
function changeFooterImage(index) {
  footerImage.style.background = images[i];

  for (let j = 0; j < paginationsİtems.length; j++) {
    if (j === index) {
      paginationsİtems[j].classList.add("is-active");
    } else {
      paginationsİtems[j].classList.remove("is-active");
    }
  }
}
